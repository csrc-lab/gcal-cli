...
time_t TimeParse::getLocalTimezoneOffset() {
    ...
    return std::mktime(&local_tm) -
           std::mktime(&utc_tm);  // Return the difference in seconds and use +,
...
std::string TimeParse::castToRFC3339(const std::tm &dateTime) {
    ...
    // Format the timezone offset, assuming your system's timezone is what you
    // want to express
    oss << (timezoneOffset >= 0 ? "+" : "-") << std::setw(2)
        << std::setfill('0') << std::abs(timezoneOffset) / 3600 << ":00";
